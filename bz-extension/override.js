function insertBzCode(v){if(v){console.log('call be bg ...')}else{console.log('call for client')}window._eval={
  ...
  _getRefData:function(v,_outMap,_inMap){
    ...
    v.cs.forEach(x=>{
      if(x.k){
        let d=x.ps.find(y=>_eval._isSign(y)||y[0]==".")?_eval._exeCode(x.ps,_outMap,_inMap):x.ps.map(y=>_eval._exeCode(y,_outMap,_inMap))
        if(x.k=="("){
...
//        console.log("fill text: "+a+","+b)
  ...
  _setBZSent:function(d,_win){
    if(!window.bzTwComm && d.url&&_TWHandler._isIgnoreRequest(d.url)){
      return
    }
    if(d.i){
