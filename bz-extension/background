...
let funMap={
  ...
  ajax:function(scope,fun,data,_callback){
...
function getIframePath(_element){
  if(_element&&!_element[0].startsWith("$(BZ.TW.document.body)")&&_element[0][0]=="$"){
...
function _setCodeToContent(c,id,tabId){
  ...
  if(tabId && !_lastErrPage){
    try{
      ...
      if(id!==undefined){
        ...
        // chrome.tabs.sendMessage(_masterTabId, {scope:"console",fun:"log",data:"BZ-LOG:set code 2 to client:"+c.length,twPage:1,tab:"master",bz:1});
...
chrome.runtime.onMessage.addListener(function(_msg, t, _sendResponse) {
  ...
  *****************************************************************************************************/
  if(_msg.bg){
    return funMap[_msg.fun](_msg.bkScope,_msg.bkFun,_msg.data,t,_msg.element)
